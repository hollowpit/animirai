...
class Toonily(Scraper):
    ...
    def get_chapter(self, chapter_id: str) -> Chapter:
        ...
        # Fetch each image with proper headers to avoid Cloudflare blocks
        processed_pages = []
        for page_url in pages:
            try:
                ...
                processed_pages.append(self._prepare_image_url(page_url, url))
            except Exception as e:
                print(f"Error processing image {page_url}: {e}")
    ...
    def _prepare_image_url(self, image_url, referer_url):
        """
        Prepare image URL to ensure it can be accessed
        This helps bypass Cloudflare image protection
        """
        # For Toonily, we'll just make sure the URL is properly formed
        # In a real implementation, you might want to pre-fetch the image
        # and check for Cloudflare blocks

        # If the image is from CDN and might be blocked by Cloudflare
        if "cdn.toonily.com" in image_url:
            # For now, we're just returning the URL, but you could implement
            # a proxy system or caching layer if needed
            return image_url
